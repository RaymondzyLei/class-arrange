...
输出:
    src/data/icourseRatings.ts  Record<课堂号, 评分字符串>
"""
...
def load_icourse_ratings(path: str) -> dict[str, str]:
    """读 icourse_spider/course_rating.json，返回 (name + '#' + sorted(teachers)) -> score。"""
    with open(path, encoding="utf-8") as f:
        data = json.load(f)
    ...
    for c in data:
        ...
        if score == NO_SCORE_SENTINEL:
            continue
        if c.get("icourse-id") in TO_ABANDON_ICOURSE_IDS:
            continue
        teachers = c.get("teachers", [])
        key = c["name"] + "#" + ",".join(sorted(teachers))
        mapping[key] = score  # 后写覆盖，重复 key 时取最后一个
    return mapping


def match_sections(sections: list[dict], ic_map: dict[str, str]) -> dict[str, str]:
...
def main():
    ...
    with open(OUT_TS, "w", encoding="utf-8") as f:
        ...
        f.write("export const icourseRatings: Record<string, string> = ")
